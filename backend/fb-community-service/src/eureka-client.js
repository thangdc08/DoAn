...
export const initEureka = (appName, port) => {
  const client = new Eureka({
    instance: {
      ...
      statusPageUrl: `http://${process.env.INSTANCE_HOST || 'localhost'}:${port}/api/community/health`,
      healthCheckUrl: `http://${process.env.INSTANCE_HOST || 'localhost'}:${port}/api/community/health`,
    },
    eureka: {
