...
import {
  Badge, Button, Col, DatePicker, Input, Row, Select, Tabs, Typography,
} from 'antd';
import {
  EditOutlined, FireOutlined, PlusOutlined, SearchOutlined,
  ...
  >
    ...
    <div
      style={{
        ...
        background: 'rgba(22,163,74,0.15)', pointerEvents: 'none',
      }}
    />
...
const MatchFeed: React.FC = () => {
  ...
  const openPosts  = matchPosts.filter((p) => p.joined < p.capacity);
  const nearbyPosts = matchPosts.slice(0, 2);
          ...
          <Col xs={24} lg={16}>
            ...
            >
              <Row gutter={[10, 10]}>
                ...
                <Col>
                  <DatePicker size="large" placeholder="Ngày chơi" style={{ borderRadius: 10, width: 160 }} />
                </Col>
                <Col>
                  <Select
                    size="large"
                ...
                <Col>
                  <Button size="large" type="primary" icon={<SearchOutlined />} style={{ fontWeight: 700, borderRadius: 10 }}>
                    Tìm
                  </Button>
                </Col>
              </Row>
              ...
              items={[
                {
                  ...
                  label: (
                    <span>
                      Đang mở&nbsp;
                      <Badge count={openPosts.length} color={colors.primary} />
                    </span>
                  ),
                  children: (
                    <div style={{ paddingBottom: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {matchPosts.map((post) => <MatchCard key={post.id} post={post} />)}
                    </div>
                  ),
                },
                {
                  ...
                  children: (
                    <div style={{ paddingBottom: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {nearbyPosts.map((post) => <MatchCard key={post.id} post={post} />)}
                    </div>
                  ),
                },
                {
